...
if __name__ == '__main__':
    ...
    # ── Test DataEmbedder ─────────────────────────────────────────────────
    embedder = DataEmbedder(d_model=d_model, max_n_vars=n_vars)
    X_bits   = torch.randint(0, 2, (B, N, n_vars, 16)).float()
    output   = embedder(X_bits)

    assert output.shape == (B, N, n_vars, d_model), \
        f"Expected {(B, N, n_vars, d_model)}, got {output.shape}"
    print(f"DataEmbedder output shape: {output.shape} — OK")

    # Row permutation invariance
    # Shuffling rows must not change the output
    perm     = torch.randperm(N)
    out_perm = embedder(X_bits[:, perm, :, :])
    assert torch.allclose(output[:, perm, :, :], out_perm, atol=1e-5), \
        "DataEmbedder is not row-permutation invariant"
    print("Row permutation invariance: OK")

    # ── Test UnitEmbedder ─────────────────────────────────────────────────
    unit_embedder = UnitEmbedder(d_model=d_model)
    unit_idx      = torch.randint(0, 9, (B, n_vars, 5))
    unit_out      = unit_embedder(unit_idx)

    assert unit_out.shape == (B, n_vars, d_model)
    print(f"UnitEmbedder output shape: {unit_out.shape} — OK")

    # Same unit vector → same embedding
    unit_same       = torch.zeros(B, n_vars, 5, dtype=torch.long)
    out_same        = unit_embedder(unit_same)
    assert torch.allclose(out_same[:, 0], out_same[:, 1]), \
        "Same unit vectors should produce same embeddings"
    print("Same units → same embedding: OK")

    print("\nAll embedder tests passed.")
    print("\nNote: Row aggregation (ISAB) is in models/encoder.py")